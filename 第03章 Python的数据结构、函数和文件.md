...
![表 3-1 Python 的集合操作](https://github.com/JackieMium/py4ds_2nd_zh/raw/master/assets/chp3/3.2.png)
...
Python 支持一种被称为匿名的、或 lambda 函数。它仅**由单条语句组成，该语句的结果就是返回值。**它是通过 lambda 关键字定义的，这个关键字没有别的含义，仅仅是说“我们正在声明的是一个匿名函数”。
...
虽然你可以直接编写 `[x * 2 for x in lists]`，但是这里我们可以非常轻松地传入一个自定义运算给 `apply_to_list` 函数。
...
柯里化（currying）是一个有趣的计算机科学术语，它指的是**通过“部分参数应用”（partial argument application）从现有函数派生出新函数**的技术。例如，假设我们有一个执行两数相加的简单函数：
